...
export const HorizontalScroller: FC<HSProps> = ({ children }) => {
  ...
  const itemsRef = useRef(
    Array.from({ length: children?.length }, (_) => createRef<HTMLLIElement>())
  );
  ...
  const activateSignal = (intersectingIndex: Number) => {
    signalsRef.current.forEach((signal, i) => {
      signal.current?.classList.toggle(
        `${styles["active"]}`,
        i === intersectingIndex
      );
    });
  ...
  const theInObHook = useIntersectionTracker(
    itemsRef,
    listRef.current!,
    0.6,
    activateSignal
  ...
  return (
    <div className={styles.HS__viewport}>
      <ul data-testid="list" ref={listRef} className={styles.HS__list}>
        {children?.map(
          (child, idx): ReactElement => (
            <li
              className={styles.HS__item}
      ...
      <ul data-testid="signal" className={styles.HS__signalList} aria-hidden>
        {Array.from({ length: children?.length }, (_, idx) => (
          <li
            key={idx}
            ref={signalsRef.current[idx]}
            className={cx(styles.HS__signal, "")}
          ></li>
        ))}
      </ul>
    </div>
